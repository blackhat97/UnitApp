...
export class DetailsPage implements OnInit {
  ...
  ionViewWillEnter(){
    ...
    this.getapi.measureResult(this.resultId).subscribe((res) => {
      ...
      const lll = byteLength(this.results[0].cname) +
                byteLength(this.results[0].ct_name) +
                byteLength(this.results[0].sname) +
                byteLength(this.results[0].memo) +
                byteLength(`${(this.results[0].datetime).split(/[T.]+/)[0]} ${(this.results[0].datetime).split(/[T.]+/)[1]}`) +
                byteLength(this.results[0].mixnum);
                //byteLength(NumberStr((this.results[0].w1_unit).toString(), 6) + NumberStr((this.results[0].w1_density).toString(), 7));
