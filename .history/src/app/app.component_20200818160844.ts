    ...
    this.platform.backButton.subscribe(async () => {
      ...
      this.routerOutlets.forEach((outlet: IonRouterOutlet) => {
        ...
        if (outlet && outlet.canGoBack()) {
          outlet.pop();
        } else if (this.router.url === '/app/tabs/tab1' || this.router.url === '/app/tabs/tab3' || this.router.url === '/app/tabs/tab4' || this.router.url === '/app/tabs/tab5' || this.router.url === '/login' ) {
  
          if (new Date().getTime() - this.lastTimeBackPress < this.timePeriodToExit) {

            let alert = this.alertCtrl.create({
              header: this.translate.instant("EXIT_APP"),
              message: this.translate.instant("EXIT_QUESTION"),
              buttons: [
                {
